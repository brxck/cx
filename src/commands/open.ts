...
export const openCommand = defineCommand({
  ...
  async run({ args }) {
    ...
    const requestedIn = parseDestination(args.in as string | undefined);
    const splitDir = parseSplitDir(args.split as string | undefined);

    let app: TaggedApp;
    const requestedTarget = args.target as string | undefined;
    if (requestedTarget) {
      app = resolveApp(tagged, requestedTarget);
    } else {
